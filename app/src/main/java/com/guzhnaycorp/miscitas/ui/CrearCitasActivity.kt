...
class CrearCitasActivity : AppCompatActivity() {
        ...
        btnNext2.setOnClickListener {
            when {
                etScheduledDate.text.toString().isEmpty() -> {
                    etScheduledDate.error = getString(R.string.validate_cita_fecha)
                }
                selectedTimeRadioBtn == null -> {
                    Snackbar.make(createCitaLinearLayout,
                        R.string.validate_cita_hora, Snackbar.LENGTH_SHORT).show()
                }
                else -> {
                    mostrarCitaDatosAConfirmar()
    ...
    private fun loadHours(doctorId: Int, date: String){
        ...
        call.enqueue(object: Callback<Schedule> {
            override fun onFailure(call: Call<Schedule>, t: Throwable) {
                Toast.makeText(this@CrearCitasActivity,getString(R.string.error_loading_hours), Toast.LENGTH_SHORT).show()
            }
            ...
            override fun onResponse(call: Call<Schedule>, response: Response<Schedule>) {
                if (response.isSuccessful) { // 200..300
                    //response.body()?.let {
                        ...
                        //Toast.makeText(this@CrearCitasActivity, "morning: ${schedule?.morning?.size}, afternoon: ${schedule?.afternoon?.size}", Toast.LENGTH_SHORT).show()
    ...
    private fun loadEspecialidades()
    {
        ...
        call.enqueue(object: Callback<ArrayList<Specialty>> {
            override fun onFailure(call: Call<ArrayList<Specialty>>, t: Throwable) {
                Toast.makeText(this@CrearCitasActivity,getString(R.string.error_loading_specialty), Toast.LENGTH_SHORT).show()
                finish()
            }
            ...
            override fun onResponse(call: Call<ArrayList<Specialty>>, response: Response<ArrayList<Specialty>>) {
                if (response.isSuccessful) { // 200..300
                    response.body()?.let {
                    val especialidades = it.toMutableList()

                    spinnerEspecialidades.adapter = ArrayAdapter<Specialty> (
                        this@CrearCitasActivity,
    ...
    private fun loadDoctors(specialtyId: Int){
        ...
        call.enqueue(object: Callback<ArrayList<Doctor>> {
            override fun onFailure(call: Call<ArrayList<Doctor>>, t: Throwable) {
                Toast.makeText(this@CrearCitasActivity,getString(R.string.error_loading_doctors), Toast.LENGTH_SHORT).show()
            }
    ...
    private fun mostrarCitaDatosAConfirmar(){
        tvConfirmDescription.text = etDescription.text.toString()
        tvConfirmEspecialidad.text = spinnerEspecialidades.selectedItem.toString()
    ...
    private fun displayIntervalRadios(hours: ArrayList<String>) {
        ...
        if (hours.isEmpty()) {
            tvHoraNoDisponible.visibility = View.VISIBLE
            return
        }

        tvHoraNoDisponible.visibility = View.GONE
